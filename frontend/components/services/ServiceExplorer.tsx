...
import { Grid3X3, List, RotateCcw, Search, X } from "lucide-react";
  ...
  return <div className="dialog-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    <div className="retro-window service-dialog" role="dialog" aria-modal="true" aria-labelledby="service-dialog-title" ref={dialogRef}>
      ...
      <div className="service-dialog__scroll">
        <div className="service-dialog__hero"><div className="pixel-frame"><Image src={service.image} alt={service.imageAlt} width={720} height={540} /></div><div><p className="section-lede">{service.shortDescription}</p><div className="tag-cloud" style={{ marginTop: "1rem" }}>{service.categories.map((category) => <span className="retro-tag" key={category}>{categoryLabels[category]}</span>)}</div></div></div>
...
export function ServiceExplorer() {
  ...
  return <>
    <section className="retro-window services-intro"><div className="retro-titlebar"><h1 className="retro-titlebar__title">Services — Galerie Astrapio</h1><div className="retro-titlebar__controls" aria-hidden="true"><span>_</span><span>□</span><span>×</span></div></div><div className="retro-window__body"><h2 className="section-heading">Services</h2><p className="section-lede">Explorez les façons dont Astrapio peut relier vos outils, vos données et vos équipes.</p><label className="service-search"><Search aria-hidden="true" /><span className="sr-only">Rechercher un service</span><input value={query} onChange={(event) => setQuery(event.target.value)} onBlur={() => updateUrl({ query })} onKeyDown={(event) => { if (event.key === "Enter") updateUrl({ query }); }} placeholder="Rechercher un service..." /></label></div></section>
